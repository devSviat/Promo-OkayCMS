...
```bash
git clone https://github.com/devSviat/Promo-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/Promo/     ← модуль
└── tests/Modules/Sviat/Promo/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/Promo
cp -r ~/Promo-OkayCMS/tests/. tests/Modules/Sviat/Promo/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/Promo
```
...
2. Неймспейс — `Modules\Sviat\Promo`.
...
use Okay\Modules\Sviat\Promo\Services\SomeService;
